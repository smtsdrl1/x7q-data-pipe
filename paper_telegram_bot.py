...
class PaperTradingBot:
    ...
    async def cmd_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            "/yardim — Bu mesaj\n\n"
            "<b>🔧 Gelişmiş Komutlar</b>\n"
            "/cbstop — Circuit breaker tetikle\n"
            "/cbresume — Circuit breaker sıfırla\n"
            "/kapat BTCUSDT — Pozisyon kapat\n"
            "/system — Sistem durum raporu\n"
            "/mc — Monte Carlo risk analizi",
            parse_mode="HTML",
        )
    ...
    # ==================== YENİ: GELİŞMİŞ KOMUTLAR ====================

    async def cmd_cb_stop(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """/cbstop — Circuit breaker manuel tetikle."""
        if not self.is_authorized(update.effective_chat.id):
            return
        try:
            ...
            await update.message.reply_text(
                "🔴 <b>Circuit Breaker TETİKLENDİ</b>\n"
                "Tüm yeni pozisyon açmalar engellendi.\n"
                "Devam ettirmek için /cbresume kullanın.",
                parse_mode="HTML",
            )
        except ImportError:
            await update.message.reply_text("⚠️ Circuit breaker modülü bulunamadı.", parse_mode="HTML")

    async def cmd_cb_resume(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """/cbresume — Circuit breaker manuel sıfırla."""
        if not self.is_authorized(update.effective_chat.id):
            return
        try:
            from utils.circuit_breaker import circuit_breaker
            ...
            await update.message.reply_text(
                "🟢 <b>Circuit Breaker Sıfırlandı</b>\n"
    ...
    async def cmd_close_position(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """/kapat BTCUSDT — Belirli bir pozisyonu kapat."""
        if not self.is_authorized(update.effective_chat.id):
            return

        if not context.args:
            await update.message.reply_text(
                "Kullanım: /kapat BTCUSDT\nÖrnek: /kapat BTCUSDT", parse_mode="HTML"
            )
            return

        symbol = context.args[0].upper()
        pos_manager = self.engine.position_manager
        if symbol not in pos_manager.open_positions:
            await update.message.reply_text(
                f"❌ <b>{symbol}</b> için açık pozisyon bulunamadı.", parse_mode="HTML"
            )
            return

        # Mevcut fiyatı al
        try:
            df = await self.engine.data_fetcher.fetch_ohlcv(symbol, limit=2)
            price = float(df["close"].iloc[-1]) if not df.empty else 0
        except Exception:
            price = pos_manager.open_positions[symbol].entry_price

        result = pos_manager.close_position(symbol, price, reason="manual_telegram")
        pnl = result.get("pnl", 0)
        emoji = "✅" if pnl >= 0 else "❌"
        await update.message.reply_text(
            f"{emoji} <b>{symbol} pozisyon kapatıldı</b>\n"
            f"  Fiyat: {price:.6f}\n"
            f"  P&amp;L: {pnl:.4f} USDT",
            parse_mode="HTML",
        )
    ...
    async def cmd_system_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """/system — Tam sistem durum raporu (CB, session, regime)."""
        if not self.is_authorized(update.effective_chat.id):
            return

        lines = ["<b>🖥 SİSTEM DURUM RAPORU</b>", "─" * 32]

        # Circuit breaker
        try:
            from utils.circuit_breaker import circuit_breaker
            cb_ok, cb_reason = circuit_breaker.check()
            cb_status = "🟢 AKTİF" if cb_ok else f"🔴 DEVRE DIŞI ({cb_reason})"
            lines.append(f"Circuit Breaker: {cb_status}")
        except Exception:
            lines.append("Circuit Breaker: N/A")

        # Session
        try:
            from utils.session_killzone import get_current_session
            sess = get_current_session()
            lines.append(
                f"Session: {sess['session']} "
                f"(Kalite: {sess['quality']}/6)"
            )
        except Exception:
            lines.append("Session: N/A")

        # Regime
        lines.append(f"Bot çalışıyor: {'✅' if self.engine.is_running else '⏹'}")
        lines.append(f"Açık pozisyon: {len(self.engine.position_manager.open_positions)}")
        lines.append(f"Günlük trade: {self.engine.risk_manager.daily_trades}")

        await update.message.reply_text("\n".join(lines), parse_mode="HTML")

    async def cmd_monte_carlo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """/mc — Monte Carlo risk simülasyonu çalıştır."""
        if not self.is_authorized(update.effective_chat.id):
            return

        await update.message.reply_text("⏳ Monte Carlo simülasyonu çalıştırılıyor...", parse_mode="HTML")

        try:
            from utils.monte_carlo import run_monte_carlo, format_mc_report
            risk = self.engine.risk_manager
            closed_trades = [t for t in risk.trade_history if t.status == "closed"]

            if len(closed_trades) < 20:
                await update.message.reply_text(
                    f"⚠️ Monte Carlo için en az 20 kapalı trade gerekli "
                    f"(şu an: {len(closed_trades)})",
                    parse_mode="HTML",
                )
                return
            ...
            result = run_monte_carlo(
                ...
                initial_capital=risk.current_capital,
                n_simulations=3000,
                n_trades=50,
            )
            report = format_mc_report(result)
            # ASCII raporu HTML'e çevir
            report_html = "<pre>" + report + "</pre>"
            await update.message.reply_text(report_html, parse_mode="HTML")

        except Exception as e:
            await update.message.reply_text(f"❌ MC hatası: {e}", parse_mode="HTML")
    ...
    async def run(self):
        ...
        # Yeni komutlar
        self.app.add_handler(CommandHandler("cbstop", self.cmd_cb_stop))
        self.app.add_handler(CommandHandler("cbresume", self.cmd_cb_resume))
        self.app.add_handler(CommandHandler("kapat", self.cmd_close_position))
        self.app.add_handler(CommandHandler("system", self.cmd_system_status))
        self.app.add_handler(CommandHandler("mc", self.cmd_monte_carlo))
        ...
        # Paper trading engine'i başlat — hataları yakala
        async def run_engine():
            try:
                await self.engine.start()
            except Exception as e:
                logger.error(f"ENGINE HATASI: {e}", exc_info=True)
                import traceback
                traceback.print_exc()
